...
function heading_autonumbering() {
  ...
  var t6 = 0;
  $(".markdown-body")
      .find("h1,h2,h3,h4,h5,h6").each(function (index) {
          let tagLevel = parseInt(this.tagName.slice(1));
          if(tagLevel == 1){
              t1++;
              t2 = 0;
              t6++;
              $(this).html((t1) + '. ' + $(this).html());
          } else if(tagLevel ==2) {
              t2++;
              t3 = 0;
              t6++;
              $(this).html((t1) + '.' + (t2) + ' ' + $(this).html());
          } else if(tagLevel == 3) {
              t3++;
              t4 = 0;
              t6++;
              $(this).html((t1) + '.' + (t2) + '.' + (t3) + " " + $(this).html());
          } else if(tagLevel == 4) {
              t4++;
              t5 = 0;
              t6++;
              $(this).html((t1) + '.' + (t2) + '.' + (t3) + '.' + (t4)  + " " + $(this).html());
          } else if(tagLevel == 5) {
              t5++;
              t6= 0;
              t6++;
              $(this).html((t1) + '.' + (t2) + '.' + (t3) + '.' + (t4) + '.' + (t5)  + " " + $(this).html());
          } else if(tagLevel == 6) {
              t6++;
              $(this).html((t1) + '.' + (t2) + '.' + (t3) + '.' + (t4) + '.' + (t5) + '.' + (t6) + " " + $(this).html());
          }
      }); // end h1
}
